...
int VisualOdometry::run(std::string datasetPath)
{
    ...
    std::filesystem::path imagesetPath =
    ...
    char filename1[200];
    char filename2[200];
    sprintf(filename1, (imagesetPath / "image_0/%06d.png").c_str(), 0);
    sprintf(filename2, (imagesetPath / "image_0/%06d.png").c_str(), 1);
    ...
    double focal;
    cv::Point2d pp;
    getCalibrationData(imagesetPath, focal, pp);

    /* Recovering the pose and the essential matrix */
    cv::Mat E, R, t, mask;

    E = cv::findEssentialMat(points2, points1, focal, pp, cv::RANSAC, 0.999, 1.0,
                         mask);
    ...
    std::filesystem::path datasetPosesPath =
        std::filesystem::path(datasetPath) /
        "data_odometry_poses/dataset/poses/00.txt";

    for (int frameNum = 2; frameNum < MAX_FRAME; ++frameNum)
    {
        sprintf(filename, (imagesetPath / "image_0/%06d.png").c_str(),
                frameNum);
        ...
        E = cv::findEssentialMat(currFeatures, prevFeatures, focal, pp,
        ...
        sprintf(text, "Coordinates: x = %02fm y = %02fm z = %02fm",
                tf.at<double>(0), tf.at<double>(1), tf.at<double>(2));
        putText(traj, text, textOrg, fontFace, fontScale, cv::Scalar::all(255),
                thickness, 8);
